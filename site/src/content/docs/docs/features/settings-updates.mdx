...
**Settings** covers seven sections:
...
- **Server control:** restart the hub process or view the last lines of `logs/stdout.log` and `logs/stderr.log` without leaving the browser.
- **Advanced:** miscellaneous server-side knobs.
...
**Restart the server:** in the Server control section, click **Restart**. The hub process restarts via the OS service manager (LaunchAgent on macOS, systemd on Linux). The browser reconnects automatically.
