...
- **Copy/paste on the iOS PWA:** the installed iPhone/iPad app has no hardware Cmd+C/V, and tmux mouse-mode blocks normal text selection, so the terminal touch-bar adds two dedicated keys. **Paste** reads the clipboard on tap (iOS shows its own paste prompt) and sends it with bracketed-paste. **Copy** starts on long-press-then-drag to select text (a normal swipe still scrolls); release the drag and tap **Copy** to copy the selection.
