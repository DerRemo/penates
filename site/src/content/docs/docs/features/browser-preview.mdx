...
Only one preview is active at a time. Apps with hardcoded origin URLs may misroute because the hub does not rewrite response bodies. HMR clients that reject the proxied origin require `server.hmr.clientPort: 443` (or equivalent) in the dev project configuration. The feature requires a working Cloudflare tunnel with the `preview` CNAME and public hostname configured; it does not work on a plain local network without those. The [iOS simulator panel](/docs/features/ios-simulator) shares the same slot, so web preview and simulator cannot be open simultaneously.
