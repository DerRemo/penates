...
That's it. The `git pull` brings in new code, `npm install` syncs any new or changed dependencies, and `launchctl kickstart -k` replaces the running process with the updated one without removing the LaunchAgent.
