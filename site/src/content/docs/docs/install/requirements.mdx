...
The hub also spawns **Codex** (OpenAI), **Antigravity** (Google `agy`), and **opencode**. These are optional. A missing CLI causes only that CLI's sessions to exit with a "not in PATH" error; it does not affect other sessions.
...
opencode is installed from npm as `opencode-ai`; the Update Center can later upgrade it with `opencode upgrade`.
