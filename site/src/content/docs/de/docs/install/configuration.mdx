...
Alle Einstellungen stehen in `~/penates/.env`. `setup.sh` erstellt diese Datei mit sinnvollen Standardwerten — du musst sie nur anfassen, wenn du optionale Features aktivieren oder den Port ändern willst.
...
| `AUTH_TOKEN` | — | **Pflichtfeld.** Bearer-Token für alle Requests. Wird von `setup.sh` generiert. Nachschlagen mit `grep AUTH_TOKEN ~/penates/.env`. |
...
| `TMUX_MOUSE` | `on` | tmux-Mouse-Mode. Auf `off` setzen, wenn du Moshi nutzt und native Touch-Selektion im Terminal willst — dann ist Scroll-Wheel in Hub-Terminals nicht verfügbar. |
...
| `VAPID_SUBJECT` | — | Pflicht für Apple Web Push. Muss eine echte HTTPS-Domain sein (kein localhost). Beispiel: `https://penates.deinedomain.xyz`. |
| `PREVIEW_DOMAIN` | — | Basis-Domain für den Browser-Preview-Proxy. Wenn gesetzt, proxied `preview.<PREVIEW_DOMAIN>` einen lokalen Dev-Server. Leer lassen um das Feature zu deaktivieren. |
| `CF_ACCESS_TEAM_DOMAIN` | — | Cloudflare-Zero-Trust-Team-Domain, z.B. `deinteam.cloudflareaccess.com`. Leer = CF-Access-JWT-Validierung deaktiviert. |
| `CF_ACCESS_AUD` | — | Cloudflare-Access-Application-Audience-Tag (64-Zeichen-Hex-String aus dem Dashboard). Beide `CF_ACCESS_*`-Variablen müssen zusammen gesetzt sein. |
...
| `WHISPER_MODEL` | — | Pfad zu einer `.gguf`-Whisper-Modelldatei. Pflicht für Voice-Input. |
