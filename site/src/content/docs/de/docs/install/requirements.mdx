...
Der Hub startet auch **Codex** (OpenAI), **Antigravity** (Google `agy`) und **opencode**. Diese sind optional. Eine fehlende CLI lässt nur die eigenen Sessions mit einem „nicht im PATH"-Hinweis sterben, ohne andere Sessions zu stören.
...
opencode wird per npm als `opencode-ai` installiert; das Update-Center kann es später mit `opencode upgrade` aktualisieren.

<Aside type="note">
Der Hub fügt `~/.local/bin`, `/opt/homebrew/bin` und `/usr/local/bin` beim Spawnen von Sessions zur Laufzeit zum PATH hinzu. Wenn ein CLI nach der npm-Installation in einem dieser Pfade liegt, wird es gefunden. Die Profil-Änderung wird trotzdem empfohlen, damit Terminal-Sessions außerhalb des Hubs funktionieren.
</Aside>
