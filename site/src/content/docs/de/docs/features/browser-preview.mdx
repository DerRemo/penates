...
Das Feature ist inaktiv, wenn `PREVIEW_DOMAIN` nicht in `.env` gesetzt ist. Es benötigt einen Cloudflare-DNS-Wildcard-Eintrag (`*.<domain>`) und einen Public Hostname, der auf `localhost:<PORT>` zeigt.
