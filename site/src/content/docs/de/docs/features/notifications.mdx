...
**Aktivitätszustand prüfen:** Der farbige Punkt auf jeder Session-Karte zeigt den aktuellen Zustand: working (blau), waiting (bernstein), idle (grau). Wurde in den letzten 60 Sekunden kein Hook ausgelöst, fällt der Zustand auf `unknown` zurück und die Karte zeigt "Aktiv".
