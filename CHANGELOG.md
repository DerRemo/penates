...
- [x] Board-Spawn-Priming auf argv umgestellt: Prompt als initiales CLI-Argument via PENATES_PRIME_PROMPT-Env-Var statt send-keys-Tippmaschinerie — primeSession durch schlanken Trust-Gate-Watchdog ersetzt {priority: p1, theme: board}
...
**Worktree-Isolation für P4-Implement-Agenten.** Die vom Board gespawnten „Implement"-Agenten (`claude --dangerously-skip-permissions`) liefen bisher im **Haupt-Checkout** des Projekts und legten dort selbst `git checkout -b idea/<slug>` an. Branch/HEAD sind aber pro Arbeitsverzeichnis — also landete jeder zweite Committer (paralleler Agent / der Mensch) auf dem ausgecheckten Idea-Branch (belegter 3-fach-Kollisions-Vorfall: Board-Fix-Commits auf `idea/animationen` statt `main`). Jetzt legt der Hub **vor** dem Spawn einen eigenen git-Worktree + Branch `idea/<slug>` an und spawnt den Agenten dort (`dir=wtPath`) — eigener Checkout, eigener HEAD, physisch getrennt, Kollision strukturell unmöglich. Neues Express-freies `lib/worktree.js` (Muster wie `git-finish.js`, alle git-Calls `execFileSync` argv, kein Shell-Interp): `worktreePathFor` (deterministischer Geschwister-Pfad `<parent>/.penates-worktrees/<projektordner>/<slug>`, nach Projektname genested gegen Slug-Kollisionen), `canIsolate` (git-repo-gated — Nicht-Git-Projekte fallen sauber auf das heutige Verhalten zurück), `ensureWorktree` (idempotent: frisch `-b` anlegen / an bestehenden Branch anhängen / vorhandenen Worktree reusen, plus Self-Heal eines stale Dirs ohne `.git`), `removeWorktree` (Disk + git-Admin, **Branch bleibt unangetastet**) und `deleteBranch` (safe `-d`, nur nach Merge). **Konservativer Lifecycle:** Session-Ende (Kill *oder* Finish) entfernt den Worktree, aber der Branch überlebt einen Kill — die Commits bleiben, Re-Implement hängt via `ensureWorktree` wieder an den bestehenden Branch an (kein Arbeitsverlust); nur der **erfolgreiche Finish** löscht den (jetzt gemergten) Branch. Cleanup hängt in den async-Routen (`DELETE /api/sessions/:name` ist jetzt async + ruft `cleanupWorktreeForSession`; die finish-Route entfernt den Worktree und löscht dann den Branch — `removeWorktree` **vor** `deleteBranch`, da git das Löschen eines ausgecheckten Branches verbietet) plus einer **Boot-Reconciliation** (Worktrees von Karten ohne lebende tmux-Session werden beim Start zurückgeholt; nur bekannte Karten, kein FS-Scan, Branch bleibt). Neues Card-Feld `worktreePath` als Cleanup-Anker (`board.js`-Whitelist + neuer `findCardBySessionRef`-Lookup); das Implement-Priming ist auf `buildImplementPriming(card, {isolated})` parametrisiert (isoliert: „du bist schon auf dem Branch im frischen Worktree — installiere zuerst die Deps, kein `checkout -b`"; Fallback Wort-für-Wort wie bisher für Nicht-Git-Projekte). **Schöne Folge:** weil der Haupt-Checkout während der Umsetzung auf `base` bleibt, nimmt `finishCard` jetzt den In-Place-Merge-Pfad (`primary===base`) statt seines eigenen Temp-Worktrees — **keine Änderung an `git-finish.js` nötig**. **Bewusste Verhaltensänderung:** der Finish löscht den gemergten `idea/<slug>` jetzt (vorher blieb er liegen) — safe `-d`, also no-op auf unmergte Branches. Verifikation: Backend-Unit **371/371** + Frontend **39/39** grün (inkl. neuem hermetischem `lib/worktree.test.js` gegen Temp-Repos); eine Integrations-Verkettung von `worktree.js` + `git-finish.js` exakt entlang implement→work→kill→re-attach→preflight→finish→cleanup (**18/18**, beweist die „Haupt-Checkout-bleibt-auf-base → In-Place-Merge"-Eigenschaft, das Branch-überlebt-Kill-Verhalten und die origin-Push-Kette); realer Server-Boot mit isoliertem HOME (`GET /api/sessions` + `/api/board/cards` → 200, Boot-Reconciliation lief ohne Crash). Der **live `claude`-Spawn** der implement-Route wurde in der automatisierten Verifikation bewusst **nicht** ausgeführt (würde einen echten autonomen Agenten starten) — die Route-Verdrahtung ist über `node --check`, zweistufige Code-Reviews pro Task und die Integrations-Kette (die exakt die Primitiven der Route durchspielt) abgesichert. Subagent-driven gebaut (11 Commits, je Implement + Spec-Review + Code-Quality-Review). Nebenbei zwei pre-existing Test-Fails auf `main` grün gemacht: `tokens.test.js` (`--ease-in` war als toter Token entfernt → `--ease-drawer`) und `usage-limits.test.js` (zeit-brittle Legacy-Punkt → `now` gepinnt). Zwei der pre-existing Follow-ups direkt nachgezogen: Session-Rename zieht `card.sessionRef` jetzt nach (`board.renameSessionRef`, in die Rename-Route verdrahtet), und ein Titel, der keinen gültigen git-Branch ergibt (`a..b`, `x.lock`-Suffix), liefert sauber **400** statt 500 (`isValidImplementBranch`-Gate in der implement-Route). Offen bleibt nur: identische Title-Slugs teilen Worktree/Branch (deterministischer Name; self-healt via Boot-Reconciliation).
...
**Notifications-Theme.** Zentrales v0.4.0-Feature: der Hub meldet jetzt zuverlässig wenn eine Session Aufmerksamkeit braucht. Attention-Detection läuft rein über Claude-Code-Hooks — `Stop`, `Notification`, `UserPromptSubmit`, `SubagentStop`, `SessionStart`, `SessionEnd` feuern per curl an `POST /api/hooks/:event`, der Hub hält Session-Activity-State in `lib/attention.js` mit 60s-Frische-Window und 10s-Cool-Down zum Entrauschen. Die vorherige Regex-basierte `tmux capture-pane`-Parser-Variante war als Fallback dabei und wurde im Lauf von v0.4.0 komplett entfernt (Hook-only). `setup.sh` installiert den Hook-Block idempotent in `~/.claude/settings.json` per `jq`-Merge mit `_owner`-Sentinel, damit Re-Runs User-Hooks nicht überschreiben. Env-Injection via `tmux new-session -e` vererbt `PENATES_SESSION`, `PENATES_URL`, `PENATES_TOKEN` an den Claude-Kindprozess; ein Rename-Alias-Map im Server sorgt dafür dass Hook-Posts auch nach `tmux rename-session` noch zur richtigen Session finden.

**Notification-Kanäle.** Notifications werden auf mehreren Kanälen ausgegeben: **Sound-Alert** in der Terminal-View (Web-Audio-Beep, togglebar), **visuelle Card-Flash plus Unread-Badge** im Dashboard, **Per-Session-Stummschalten** über eine Mute-Toggle auf der Session-Card, und **Web-Push API** als zweiter persistenter Kanal neben den In-App-Signalen (Service-Worker + VAPID, Subscription-Registry unter `~/.penates/push-subscriptions.json`). Eine separate Zuverlässigkeits-Welle hat iOS-APNs-Quirks gefixt (Subject-Format, BadJwtToken), den Attention-Gate von tmux-attached auf echte **Per-Device-Presence** umgestellt (Frontend meldet via WS welche Sessions es gerade sieht), und doppelte VAPID-Loader-Loads gehärtet. In-App-Notifications werden supprimiert wenn die Session im Foreground ist, damit keine Lärm-Duplikate durch parallele Kanäle entstehen.
...
Tab-Titel mit `(N) Penates`. Wird bei jedem Session-Polling
...
`~/.penates/projects.json`, Scan-Roots per `PROJECT_ROOTS`-Env
...
`~/.penates/sessions.json` persistiert jede jemals erstellte
