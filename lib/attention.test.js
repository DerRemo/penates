...
test('SessionEnd broadcasts session-ended before forgetting', () => {
  attention.stop();
  const events = [];
  attention.subscribe((e) => events.push(e));
  attention.reportHookEvent('cc-demo', 'UserPromptSubmit', {}, 1000);
  attention.reportHookEvent('cc-demo', 'SessionEnd', {}, 2000);
  const ended = events.filter((e) => e.type === 'session-ended');
  assert.equal(ended.length, 1);
  assert.deepEqual(ended[0], { type: 'session-ended', name: 'cc-demo', at: 2000 });
  attention.stop();
});

test('forget() broadcasts session-ended only when the session existed', () => {
  attention.stop();
  const events = [];
  attention.subscribe((e) => events.push(e));
  attention.forget('cc-missing');
  attention.reportHookEvent('cc-live', 'UserPromptSubmit', {}, 1000);
  attention.forget('cc-live');
  const ended = events.filter((e) => e.type === 'session-ended');
  assert.equal(ended.length, 1);
  assert.equal(ended[0].name, 'cc-live');
  attention.stop();
});
