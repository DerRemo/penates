...
test('reportHookEvent — attached-Session broadcastet session-activity aber nicht session-attention', () => {
  reset();
  attention.reportHookEvent('cc-a', 'Stop', {}, 500); // Init state
  attention.setAttached('cc-a', true);
  const { events, activities } = collect();
  attention.reportHookEvent('cc-a', 'Notification', {}, 1000);
  assert.equal(events.length, 0);
  assert.equal(activities.length, 1);
  assert.equal(attention._debugState().sessions[0].activity, 'waiting');
});
