...
test('a known session with a non-string directory degrades without crashing', () => {
  const r = compose({
    known: [knownEntry({ name: 'cc-bad', directory: { isTrusted: true } })],
    projects: [{ id: 'p1', path: '/proj', displayName: 'Proj' }],
    getGitStatus: (path) => {
  ...
  assert.equal(r.length, 1);
  assert.equal(r[0].status, 'dormant');
  assert.equal(r[0].path, null);
  assert.equal(r[0].project, null);
});
