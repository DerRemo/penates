...
export async function writeStream(projectRoot, parentRel, name, stream, { onConflict = 'rename' } = {}) {
  ...
  // pid + random im tmp-Namen: zwei parallele Overwrite-Uploads auf denselben
  // Zielnamen (Busboy verarbeitet bis zu 50 Files gleichzeitig) dürfen sich nicht
  // dieselbe tmp-Datei teilen — sonst clobbern sich die Write-Streams.
  const tmp = `${targetAbs}.penates-upload-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  try {
