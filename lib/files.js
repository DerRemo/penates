...
import { resolveTrash } from './platform.js';
...
// Klassifiziert einen File-I/O-Fehler in HTTP-Status + Body-Code + ob er
// "unexpected" ist (→ Caller soll einen Stack loggen). Wichtig: ein roher
// ENOENT (verschwundener Pfad — z.B. ein aufgeräumter Worktree, den das
// Frontend noch pollt) ist KEIN Internal Error, sondern ein 404 — sonst
// flutet jeder Poll die Logs mit „[files] unexpected"-Stacks.
export function classifyFileError(err) {
  if (err instanceof FileError) {
    return { status: FILE_ERROR_STATUS[err.code] || 500, code: err.code, unexpected: false };
  }
  if (err && err.code === 'ENOENT') {
    return { status: 404, code: 'not-found', unexpected: false };
  }
  return { status: 500, code: 'internal', unexpected: true };
}
...
export async function deleteToTrash(projectRoot, relPaths) {
  if (!relPaths.length) return { count: 0 };  // trash ohne Argumente würde sonst erroren
  const absPaths = relPaths.map(p => resolveSafe(projectRoot, p));
  // OS-spezifische Trash-Auflösung (macOS: trash/usr-bin-trash; Linux: gio/trash-put).
  const trash = resolveTrash();
  if (!trash) {
    // Ehrlicher Fehler — NIEMALS auf rm zurückfallen. Minimale Linux-Server-Images
    // haben oft weder gio noch trash-cli; dem User sagen, wie er das behebt.
    throw new FileError('trash-unavailable',
      'No trash tool found. Install gio (glib2) or trash-cli (provides trash-put).');
  }
  try {
    await execFile(trash.bin, [...trash.args, ...absPaths]);
  } catch (e) {
