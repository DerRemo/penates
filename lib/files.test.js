...
import { resolveSafe, listDir, FileError, readFile, streamFileToResponse, createEmptyFile, writeStream, classifyFileError } from './files.js';

test('classifyFileError: FileError maps to its status, not unexpected', () => {
  assert.deepEqual(classifyFileError(new FileError('forbidden', 'x')), { status: 403, code: 'forbidden', unexpected: false });
  assert.deepEqual(classifyFileError(new FileError('not-found', 'x')), { status: 404, code: 'not-found', unexpected: false });
  assert.deepEqual(classifyFileError(new FileError('oversize', 'x')), { status: 413, code: 'oversize', unexpected: false });
});
test('classifyFileError: unknown FileError code → 500 but still not unexpected', () => {
  assert.deepEqual(classifyFileError(new FileError('weird', 'x')), { status: 500, code: 'weird', unexpected: false });
});
test('classifyFileError: raw ENOENT (vanished path) → quiet 404, not a 500-with-stack', () => {
  const err = Object.assign(new Error('no such file'), { code: 'ENOENT' });
  assert.deepEqual(classifyFileError(err), { status: 404, code: 'not-found', unexpected: false });
});
test('classifyFileError: other raw error → 500 + unexpected (must be logged)', () => {
  assert.deepEqual(classifyFileError(new Error('boom')), { status: 500, code: 'internal', unexpected: true });
});
...
test('writeStream: concurrent same-name uploads both survive (no lost file)', async () => {
  // Regression: vorher wählten beide Uploads denselben Zielnamen per existsSync
  // und `rename`'ten ihre tmp-Datei blind drüber → der zweite überschrieb den
  // ersten lautlos. Jetzt beansprucht link() den Namen atomar → kein Verlust.
  const { root, cleanup } = makeTempProject({});
  try {
    const [a, b] = await Promise.all([
      writeStream(root, '', 'dup.txt', Readable.from(['AAA']), { onConflict: 'rename' }),
      writeStream(root, '', 'dup.txt', Readable.from(['BBB']), { onConflict: 'rename' }),
    ]);
    assert.deepEqual([a.name, b.name].sort(), ['dup-1.txt', 'dup.txt'], 'distinct target names');
    const real = realpathSync(root);
    const contents = [readFileSync(join(real, a.name), 'utf8'), readFileSync(join(real, b.name), 'utf8')].sort();
    assert.deepEqual(contents, ['AAA', 'BBB'], 'both upload bodies persisted');
  } finally { cleanup(); }
});
...
test('streamFileToResponse — sets headers and streams body', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'penates-dl-'));
...
// Trash resolution moved to lib/platform.js (resolveTrash) — see platform.test.js.
