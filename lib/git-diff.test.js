...
import { parseStatusV2, getDiff, gitStatusMap, getRecentCommits, detectBaseBranch, getBranchDiff, splitUnifiedDiff } from './git-diff.js';
...
test('getDiff: Diff-Body bleibt für Umlaut-/Space-Dateinamen erhalten', () => {
  const { dir, git, cleanup } = makeRepo();
  try {
    writeFileSync(join(dir, 'späce datei.txt'), 'erste zeile\n');
    git('add', '-A'); git('commit', '-q', '-m', 'init');
    writeFileSync(join(dir, 'späce datei.txt'), 'erste zeile\nzweite zeile\n');  // unstaged mod
    const r = getDiff(dir);
    const f = r.files.find(x => x.path === 'späce datei.txt');
    assert.ok(f, 'umlaut+space file present in list');
    assert.equal(f.binary, false);
    assert.equal(f.additions, 1);
    assert.ok(f.diff && f.diff.includes('zweite zeile'), 'diff body must be preserved (regression: was null)');
  } finally { cleanup(); }
});

test('getDiff: untracked Verzeichnis expandiert in einzelne Dateien (nicht binary)', () => {
  const { dir, git, cleanup } = makeRepo();
  try {
    writeFileSync(join(dir, 'tracked.txt'), 'x\n');
    git('add', '-A'); git('commit', '-q', '-m', 'init');
    mkdirSync(join(dir, 'newdir'));
    writeFileSync(join(dir, 'newdir', 'a.txt'), 'aaa\n');
    writeFileSync(join(dir, 'newdir', 'b.txt'), 'bbb\n');
    const r = getDiff(dir);
    const untracked = r.files.filter(f => f.category === 'untracked').map(f => f.path).sort();
    assert.deepEqual(untracked, ['newdir/a.txt', 'newdir/b.txt']);  // regression: war ['newdir/'] binary
    const a = r.files.find(f => f.path === 'newdir/a.txt');
    assert.equal(a.binary, false);
    assert.ok(a.diff && a.diff.includes('aaa'));
  } finally { cleanup(); }
});

test('splitUnifiedDiff: dekodiert gequotete + space-getabte Header-Pfade', () => {
  // quotepath=true-Stil: Umlaut oktal-escaped + Space-Pfad mit angehängtem Tab.
  const block =
    ...
    '+++ "b/sp\\303\\244ce.txt"\t\n' +
    '@@ -0,0 +1 @@\n+x\n';
  const map = splitUnifiedDiff(block);
  assert.ok(map.has('späce.txt'), `expected literal UTF-8 key, got ${[...map.keys()]}`);
});

test('splitUnifiedDiff: Delete (+++ /dev/null) nimmt den --- Pfad', () => {
  const block =
...
test('gitStatusMap returns a path→status map for a dirty repo', () => {
  const dir = mkdtempSync(join(tmpdir(), 'penates-gsm-'));
  try {
...
test('gitStatusMap returns null when cwd is not a git repo', () => {
  const dir = mkdtempSync(join(tmpdir(), 'penates-gsm-nr-'));
...
function makeBranchRepo() {
  const dir = mkdtempSync(join(tmpdir(), 'pen-gd-'));
  g(dir, 'init', '-b', 'main');
...
test('getBranchDiff returns isRepo:false for a non-repo dir', () => {
  const dir = mkdtempSync(join(tmpdir(), 'pen-nr-'));
