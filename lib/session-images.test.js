...
test('saveSessionImage writes a PNG into .penates-images/ with timestamp filename', () => {
  const cwd = tmpCwd();
  try {
    const { rel, abs } = saveSessionImage(cwd, PNG);
    assert.ok(rel.startsWith('.penates-images/'), `rel was ${rel}`);
    const fname = rel.slice('.penates-images/'.length);
...
test('saveSessionImage ensures .gitignore has .penates-images/ exactly once, idempotent', () => {
  const cwd = tmpCwd();
  try {
    ...
    assert.ok(/^\.penates-images\/$/m.test(after1), 'line present');
    saveSessionImage(cwd, PNG);  // second save
    const after2 = readFileSync(gi, 'utf8');
    const count = (after2.match(/^\.penates-images\/$/gm) || []).length;
...
test('saveSessionImage preserves an existing unrelated .gitignore', () => {
  ...
  try {
    ...
    assert.ok(/^\.penates-images\/$/m.test(gi), 'new line appended');
  } finally { rmSync(cwd, { recursive: true, force: true }); }
});
...
test('saveSessionImage path-guard blocks an escaping ext', () => {
  ...
  try {
    ...
    const dir = join(cwd, '.penates-images');
