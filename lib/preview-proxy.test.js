...
test('stripCachebust removes only the __cchub param', () => {
  assert.equal(stripCachebust('/?__cchub=53714'), '/');
  ...
  assert.equal(stripCachebust('/app?a=1&__cchub=5173'), '/app?a=1');
  assert.equal(stripCachebust('/app?__cchub=5173&b=2'), '/app?b=2');
...
test('proxyHttp strips __cchub so exact-req.url-match upstreams serve the index', async () => {
  ...
  try {
    ...
    const r = await get(frontPort, '/?__cchub=53714');
    assert.equal(r.status, 200);
