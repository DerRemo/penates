...
import { isPreviewHost, proxyHttp, attachUpgrade, stripCachebust } from './preview-proxy.js';

const HOST = 'preview.hub.example.com';

// ── stripCachebust: Hub-eigener Cache-Bust-Param raus, Rest unangetastet ──
test('stripCachebust removes only the __penates param', () => {
  assert.equal(stripCachebust('/?__penates=53714'), '/');
  assert.equal(stripCachebust('/'), '/');
  assert.equal(stripCachebust('/foo/bar'), '/foo/bar');
  // fremde Query-Parameter echter Dev-Server bleiben erhalten
  assert.equal(stripCachebust('/app?a=1&__penates=5173'), '/app?a=1');
  assert.equal(stripCachebust('/app?__penates=5173&b=2'), '/app?b=2');
  assert.equal(stripCachebust('/x?token=abc'), '/x?token=abc');
});
...
test('proxyHttp strips __penates so exact-req.url-match upstreams serve the index', async () => {
  // Upstream wie der Visual-Companion: matcht req.url exakt auf '/', 404 sonst.
  const target = await listen((req, res) => {
    if (req.url === '/') { res.writeHead(200); res.end('index'); }
  ...
  const frontPort = front.address().port;
  try {
    // Genau die URL, die das PreviewPanel-iframe lädt → muss jetzt 200 liefern.
    const r = await get(frontPort, '/?__penates=53714');
    assert.equal(r.status, 200);
