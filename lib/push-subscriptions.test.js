...
// In-tmp override via HOME-env. Modul erwartet ~/.penates.
const tmp = await fs.mkdtemp(join(tmpdir(), 'penates-push-'));
...
async function writeStore(data) {
  await fs.mkdir(join(tmp, '.penates'), { recursive: true });
  await fs.writeFile(
    join(tmp, '.penates', 'push-subscriptions.json'),
...
test('concurrent saves do not corrupt the store (serialized writes)', async () => {
  ...
  const raw = await fs.readFile(join(tmp, '.penates', 'push-subscriptions.json'), 'utf-8');
