...
`);
    ...
    const before = readdirSync(tmpdir()).filter((n) => n.startsWith('cch-voice-')).length;
    await assert.rejects(voice.transcribe(WAV, {}));
    const after = readdirSync(tmpdir()).filter((n) => n.startsWith('cch-voice-')).length;
    assert.equal(after, before, 'no leaked cch-voice- temp dirs');
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
