...
test('erkennt Released- und Dev-Versions-Header', () => {
  const md = ['# My Project', '', '## Released: v1.2.0', '', '## In Development: v1.3.0', ''].join('\n');
...
test('sammelt Checkbox-Items mit korrekten done-Flags, Texten und line-Nummern', () => {
  const md = [
    ...
    '- [x] Feature B',           // line 3
    '',                          // line 4
    '## In Development: v1.1.0', // line 5
    '- [ ] Feature C',           // line 6
    '- [x] Feature D',           // line 7
    '',                          // line 8
    '## Backlog / Ideas',        // line 9
    '- [ ] Idea X',              // line 10
    '- [ ] Idea Y',              // line 11
  ].join('\n');
  const r = parseRoadmap(md);
...
test('Backlog-Header ohne / Ideas Suffix wird erkannt', () => {
  const md = '## Backlog\n- [ ] plain backlog item';
  const r = parseRoadmap(md);
  assert.equal(r.backlog.length, 1);
...
test('Backlog-Header mit deutschem "/ Ideen" Suffix wird erkannt', () => {
  const r = parseRoadmap('## Backlog / Ideen\n- [ ] deutsche Idee');
  assert.equal(r.backlog.length, 1);
  assert.equal(r.backlog[0].text, 'deutsche Idee');
  assert.equal(r.unknown.length, 0);  // darf NICHT als unknown-H2 landen
});
...
test('parseRoadmap — leere Metadata-Klammern {} liefern leeres meta-Objekt', () => {
  const r = parseRoadmap('## Backlog / Ideas\n- [ ] foo {}');
