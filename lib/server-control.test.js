...
test('buildLaunchdTarget composes gui/<uid>/<label>', () => {
  assert.equal(buildLaunchdTarget(501, 'com.claude-code-hub'), 'gui/501/com.claude-code-hub');
});
...
test('tailFile returns the last N lines', async () => {
  const p = join(tmpdir(), `cch-log-${process.pid}-${Date.now()}.log`);
...
test('tailFile caps the bytes it reads (last maxBytes only)', async () => {
  const p = join(tmpdir(), `cch-big-${process.pid}-${Date.now()}.log`);
