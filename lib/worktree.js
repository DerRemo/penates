...
export function worktreePathFor(repo, slug) {
  return join(dirname(repo), '.penates-worktrees', basename(repo), slug);
}
...
// True, wenn refs/heads/<branch> existiert. Reine git-Read-Funktion, fehlertolerant.
// Für die Cleanup-Summary: nach einem `git branch -d`-Versuch unterscheidet ein
// Aufruf danach zwischen "gelöscht" (merged → weg) und "behalten" (ungemergt → -d
// verweigert, Branch bleibt). deleteBranch schluckt Fehler und kann das nicht melden.
export function branchExists(repo, branch) {
  return tryGit(repo, ['rev-parse', '--verify', '--quiet', `refs/heads/${branch}`]);
}
