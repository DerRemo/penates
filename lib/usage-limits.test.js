...
let dir, logPath;
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'penates-limits-'));
...
test('legacy points without acct map to a default account', async () => {
  ...
  // now an die Punkt-Zeit gepinnt, sonst fällt der Punkt nach 7 Tagen aus dem
  // Fenster (getLimitHistory filtert obj.t < now-days) und der Test wird flaky.
  const h = await limits.getLimitHistory({ days: 7, now: Date.parse('2026-05-30T10:00:00.000Z') });
