...
// Parst `git diff --numstat` → Map<path, {additions, deletions, binary}>.
...
export function getDiff(cwd, { maxFileBytes = 200_000, maxUntracked = 100 } = {}) {
  ...
  const unstagedNum = parseNumstat(git(cwd, ['diff', '--numstat']));
  const stagedNum = parseNumstat(git(cwd, ['diff', '--cached', '--numstat']));
