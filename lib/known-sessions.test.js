...
// Durability regression: a fire-and-forget setManuallyStopped (as destroySession
// used to do) must still hit disk before a shutdown/restart can drop it — else
// the tmux-continuum auto-restore loses its kill-switch and revives the session.
// flush() lets shutdown() drain the atomic-write queue.
test('flush() drains a fire-and-forget setManuallyStopped before reload', async () => {
  await known.load();
  await known.add({ name: 'cc-d', directory: '/tmp', command: 'claude' });
  // NOT awaited — mirrors the old fire-and-forget kill path.
  known.setManuallyStopped('cc-d', true);
  // shutdown() drains the queue here.
  await known.flush();
  // A fresh process boot reads only what reached disk.
  await known.load();
  assert.equal(known.find('cc-d').manuallyStopped, true);
});
