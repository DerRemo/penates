...
function computeResetMs(logDir, file, line) {
  ...
  if (g && fd) {
    ...
    // glog-Zeilen tragen kein Jahr: liegt der Zeilen-Monat VOR dem Datei-Monat,
    // wurde die Zeile nach einem Jahreswechsel in eine im Vorjahr angelegte Datei
    // geschrieben (z.B. Datei cli-20251231_…, Zeile I0101 …) → Jahr+1, sonst läge
    // das Event ~1 Jahr in der Vergangenheit und der Reset wäre falsch.
    let year = +fd[1];
    if (+g[1] < +fd[2]) year += 1;
    eventMs = new Date(year, +g[1] - 1, +g[2], +g[3], +g[4], +g[5]).getTime();
  } else {
