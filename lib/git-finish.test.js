...
test('finishCard on base never sweeps a concurrently-staged foreign file into a commit', () => {
  // Regression: der alte Direkt-Pfad (primary===base) machte `git add CHANGELOG`
  // + `git commit` (ohne pathspec) auf dem LIVE-Index → eine parallel gestagte
  // Fremddatei landete im Changelog-Commit. Jetzt baut der Merge isoliert im
  // Worktree → die Fremddatei kann nie in einen Commit gezogen werden.
  const { dir, origin } = makeRepoWithOrigin();
  try {
    writeFileSync(join(dir, 'foreign.txt'), 'concurrent writer\n');
    g(dir, 'add', 'foreign.txt'); // fremder git-Writer stagt im Live-Index
    try { finishCard(dir, 'idea/x', 'main', 'Feature X', changelogWriter('Feature X')); }
    catch { /* ff-only darf auf dirty index verweigern — auch das ist sicher (keine Korruption) */ }
    // Egal ob gemerged oder verweigert: foreign.txt darf in KEINEM Commit stehen.
    assert.doesNotMatch(g(dir, 'log', '--all', '--name-only', '--format='), /foreign\.txt/);
  } finally { cleanup(dir, origin); }
});
