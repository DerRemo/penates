...
import { appendFileSync, statSync, readFileSync, writeFileSync } from 'fs';
import { join } from 'path';
import { homedir } from 'os';

const HUB_DIR = join(homedir(), '.penates');
const FRESH_MS = 120_000;
const LOG_THROTTLE_MS = 5 * 60_000;
const MAX_LOG_SIZE = 5 * 1024 * 1024;

// Log-Pfad ist überschreibbar (Tests). Default: ~/.penates/usage-limits.jsonl
...
export function recordUsageSnapshot(accounts, { now = Date.now() } = {}) {
  ...
  if (lines) {
    // Append + Rotate synchron im SELBEN Event-Loop-Turn: so kann kein zweiter
    // Snapshot-Append zwischen Read und Truncate einer Rotation einschlagen
    // (verlöre sonst Zeilen) und keine zwei Rotationen überlappen. rotateMaybe
    // greift nur jenseits MAX_LOG_SIZE (throttled 5 min) — Blockierung ist hier
    // vernachlässigbar.
    try { appendFileSync(LIMITS_LOG, lines); rotateMaybe(); } catch { /* ignore write errors */ }
  }
}
...
function rotateMaybe() {
  let st;
  try { st = statSync(LIMITS_LOG); } catch { return; }
  if (st.size < MAX_LOG_SIZE) return;
  let raw;
  try { raw = readFileSync(LIMITS_LOG, 'utf8'); } catch { return; }
  const lines = raw.split('\n').filter(Boolean);
  const half = Math.floor(lines.length / 2);
  try { writeFileSync(LIMITS_LOG, lines.slice(half).join('\n') + '\n'); } catch { /* ignore */ }
}
