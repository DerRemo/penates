...
// Nur http(s), mailto, relative Pfade und Fragmente sind als href-Schema sicher.
// Die URL ist hier bereits HTML-escaped (z.B. `:` bleibt `:`, aber `"`→`&quot;`),
// das Schema-Präfix bleibt davon unberührt.
...
function renderInline(text) {
  ...
  return parts.map(p => {
    ...
    // Links [text](url) — URL wird escaped (Quotes können nicht ausbrechen) UND
    // das Schema validiert: javascript:/data:/vbscript: würden im Hub-Origin
    // (Token im localStorage) Skript ausführen. Unsicheres Schema → kein <a>,
    // der Roh-Text (bereits escaped) bleibt stehen.
