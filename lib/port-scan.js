...
// lsof liegt auf macOS in /usr/sbin, das im minimalen LaunchAgent-PATH fehlt.
// Daher mehrere Kandidaten probieren (PATH-Lookup zuerst, dann Absolutpfade).
const LSOF_CANDIDATES = ['lsof', '/usr/sbin/lsof', '/usr/bin/lsof', '/opt/homebrew/bin/lsof'];
...
export function listListeningPorts({ excludePort } = {}) {
  ...
  return []; // kein lsof gefunden → leere Liste, manuelle Port-Eingabe bleibt nutzbar
}
