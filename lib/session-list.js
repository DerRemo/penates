...
export function composeSessions({
  ...
  const dormant = known
    ...
    .map((e) => ({
      name: e.name,
      path: typeof e.directory === 'string' ? e.directory : null,
      command: e.command,
  ...
  const projectOf = (cwd) => {
    ...
    const m = projects.find((p) => typeof p.path === 'string' && (cwd === p.path || cwd.startsWith(p.path + '/')));
  ...
  const enrich = (s) => {
    ...
    base.git = getGitStatus(path);
    base.project = projectOf(path);
