...
test('isExecutable — hub/cli/ext executable, agy/dep/unknown not', () => {
  ...
  assert.equal(isExecutable('cli:opencode'), true);
...
test('updateCommandFor — returns static command, hub interpolates repoDir', () => {
  ...
  assert.equal(updateCommandFor('cli:opencode'), 'opencode upgrade');
...
test('CLI_TARGETS — declares the four CLIs with npm mapping', () => {
  ...
  assert.deepEqual(ids, ['cli:claude', 'cli:codex', 'cli:agy', 'cli:opencode']);
  ...
  assert.equal(CLI_TARGETS.find(c => c.id === 'cli:opencode').npm, 'opencode-ai');
});
...
test('collectClis — version + npm latest, agy latest:null, missing bin skipped', async () => {
  const exec = (bin) => {
    ...
    if (bin === 'opencode') throw new Error('not found'); // missing → skip
  ...
  assert.ok(!ids.includes('cli:opencode')); // missing bin dropped
...
test('createUpdates — groups payload, computes outdatedCount, caches', async () => {
  ...
  assert.equal(p.clis.length, 4);          // claude, codex, agy, opencode
...
test('scripts/update.sh — exists, sets strict mode, checks out latest tag, runs setup.sh', () => {
  const here = dirname(fileURLToPath(import.meta.url));
  const p = pjoin(here, '..', 'scripts', 'update.sh');
  assert.ok(existsSync(p), 'scripts/update.sh must exist');
  const sh = readFileSync(p, 'utf8');
  assert.match(sh, /set -euo pipefail/);
  assert.match(sh, /git fetch --tags/);
  assert.match(sh, /git checkout/);
  assert.match(sh, /setup\.sh/);
});
