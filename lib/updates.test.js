...
test('CLI_TARGETS — declares the five CLIs with npm mapping', () => {
  const ids = CLI_TARGETS.map(c => c.id);
  assert.deepEqual(ids, ['cli:claude', 'cli:codex', 'cli:gemini', 'cli:agy', 'cli:opencode']);
...
test('createUpdates — groups payload, computes outdatedCount, caches', async () => {
  ...
  assert.equal(p.clis.length, 5);          // agy + opencode included (latest:null)
