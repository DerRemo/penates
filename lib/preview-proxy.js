...
// Der Hub hängt an die iframe-URL einen eigenen Cache-Bust-Param `?__cchub=<port>`
...
// `req.url`-Match und liefern für `/?__cchub=…` ein 404 statt der Indexseite (z.B.
...
export function stripCachebust(url) {
  ...
  if (!params.has('__cchub')) return url;        // nichts zu tun
  params.delete('__cchub');
