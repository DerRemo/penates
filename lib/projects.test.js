...
// test never touches the user's real ~/.penates registry. Dynamic
// import ensures projects.js reads the env at module-eval time after we set it.
const regDir = mkdtempSync(join(tmpdir(), 'penates-reg-'));
process.env.PENATES_REGISTRY_DIR = regDir;
const { listProjects, discoverProjects, patchProject, getProject, setProjectPinned, removeProject, resolveProjectDoc } = await import('./projects.js');

test('resolveProjectDoc prefers CHANGELOG.md, falls back to ROADMAP.md', async () => {
  const a = mkdtempSync(join(tmpdir(), 'penates-doc-'));
  writeFileSync(join(a, 'ROADMAP.md'), '# x');
  assert.equal(await resolveProjectDoc(a), join(a, 'ROADMAP.md'));
  writeFileSync(join(a, 'CHANGELOG.md'), '# y');
  assert.equal(await resolveProjectDoc(a), join(a, 'CHANGELOG.md'));
  const b = mkdtempSync(join(tmpdir(), 'penates-doc-'));
  assert.equal(await resolveProjectDoc(b), null);
});

test('listProjects exposes ROADMAP.md mtimeMs', async () => {
  const root = mkdtempSync(join(tmpdir(), 'penates-proj-'));
...
test('discoverProjects registers docless folders (missing) and skips hidden', async () => {
  const root = mkdtempSync(join(tmpdir(), 'penates-nodoc-'));
  mkdirSync(join(root, 'plain-folder'));   // no CHANGELOG/ROADMAP
  mkdirSync(join(root, '.hidden'));        // hidden → must be skipped
  await discoverProjects([root]);
  const list = await listProjects();
  const plain = list.find(p => p.path === join(root, 'plain-folder'));
  assert.ok(plain, 'docless folder should now be discovered');
  assert.equal(plain.missing, true, 'docless folder is listed as missing');
  assert.ok(!list.some(p => p.path === join(root, '.hidden')), 'hidden folder skipped');
  rmSync(root, { recursive: true, force: true });
});
...
async function makeProject(content) {
  const root = mkdtempSync(join(tmpdir(), 'penates-p4-'));
...
test('setProjectPinned toggelt pinned und listProjects reicht es durch', async () => {
  const root = mkdtempSync(join(tmpdir(), 'pen-pin-'));
...
test('removeProject deregistriert das Projekt (Datei bleibt)', async () => {
  const root = mkdtempSync(join(tmpdir(), 'pen-rm-'));
...
test("patchProject action:'reorder' verschiebt Item an Index 0 (three → first)", async () => {
  const root = mkdtempSync(join(tmpdir(), 'penates-reorder-'));
...
test("patchProject action:'reorder' mit nicht-integer toIndex wirft bad-body", async () => {
  const root = mkdtempSync(join(tmpdir(), 'penates-reorder-bad-'));
