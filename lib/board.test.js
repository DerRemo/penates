...
// load() liest den (frischen, leeren) board.json unter PENATES_HOME und setzt den
// State zurück → jeder Test ist isoliert, ohne ESM-Cache-Bust nötig.
...
test('persists across reload', async () => {
  const b = await fresh();
  const c = await b.addCard({ projectId: 'p1', title: 'A' });
  await b.load();   // re-read the same board.json under PENATES_HOME
...
test('migrateBacklog: whitelist .gitignore negation is renamed to CHANGELOG.md', async () => {
  const b = await fresh();
  const proj = await fs.mkdtemp(join(tmpdir(), 'mproj-wl-'));
  await fs.writeFile(join(proj, 'ROADMAP.md'), SAMPLE);
  // Whitelist-Style: `/*` ignoriert alles, `!/ROADMAP.md` holt das Plan-Dokument
  // zurück. Nach dem Rename muss CHANGELOG.md whitelisted sein, sonst untracked.
  await fs.writeFile(join(proj, '.gitignore'), '/*\n!/README.md\n!/ROADMAP.md\n');

  await b.migrateBacklog([{ id: 'wl', path: proj }]);

  const gi = await fs.readFile(join(proj, '.gitignore'), 'utf8');
  assert.ok(/^!\/CHANGELOG\.md$/m.test(gi), 'negation renamed to !/CHANGELOG.md');
  assert.ok(!/ROADMAP\.md/.test(gi), 'no ROADMAP.md reference left in .gitignore');
  assert.ok(/^\/\*$/m.test(gi), '/* root-ignore preserved');
});
...
test('stripBacklogSection removes a German "## Backlog / Ideen" header too', async () => {
  const md = '## In Development: v1\n- [ ] wip\n\n## Backlog / Ideen\n- [ ] Idee A\n- [ ] Idee B\n\n## Changelog\nnote\n';
  const out = board.stripBacklogSection(md);
  assert.ok(!/## Backlog/.test(out), 'German backlog section must be stripped');
  assert.ok(!/Idee A/.test(out));
  assert.ok(/## In Development: v1/.test(out));
  assert.ok(/## Changelog/.test(out));
});
...
test('load() migrates legacy spec-stage cards to brainstorming', async () => {
  await fs.writeFile(join(process.env.PENATES_HOME, 'board.json'), JSON.stringify({
    migratedAt: null,
    cards: [
...
// ── worktree-isolation: worktreePath + findCardBySessionRef ─────────────────

test('updateCard persistiert worktreePath (string und null)', async () => {
  const b = await fresh();
  const c = await b.addCard({ projectId: 'p1', title: 'Foo' });
  const u = await b.updateCard(c.id, { worktreePath: '/abs/.penates-worktrees/x/foo' });
  assert.equal(u.worktreePath, '/abs/.penates-worktrees/x/foo');
  const u2 = await b.updateCard(c.id, { worktreePath: null });
  assert.equal(u2.worktreePath, null);
});

test('addCard initialisiert worktreePath auf null', async () => {
  const b = await fresh();
  const c = await b.addCard({ projectId: 'p1', title: 'Bar' });
  assert.equal(c.worktreePath, null);
});

test('findCardBySessionRef findet die Karte über sessionRef', async () => {
  const b = await fresh();
  const c = await b.addCard({ projectId: 'p1', title: 'Baz' });
  await b.updateCard(c.id, { sessionRef: 'cc-baz' });
  assert.equal(b.findCardBySessionRef('cc-baz').id, c.id);
  assert.equal(b.findCardBySessionRef('cc-nope'), null);
});

test('renameSessionRef verschiebt sessionRef von alt auf neu', async () => {
  const b = await fresh();
  const c = await b.addCard({ projectId: 'p1', title: 'X' });
  await b.updateCard(c.id, { sessionRef: 'cc-old' });
  const u = await b.renameSessionRef('cc-old', 'cc-new');
  assert.equal(u.id, c.id);
  assert.equal(b.findCardBySessionRef('cc-new').id, c.id);
  assert.equal(b.findCardBySessionRef('cc-old'), null);
});

test('renameSessionRef ist no-op (null) wenn keine Karte den alten Ref hat', async () => {
  const b = await fresh();
  const r = await b.renameSessionRef('cc-nope', 'cc-new');
  assert.equal(r, null);
});

// ── adjacentStage (Mobile-Board Quick-Move ‹ ›) ──────────────────────
test('adjacentStage: forward/back within bounds', () => {
  assert.equal(board.adjacentStage('idea', +1), 'brainstorming');
  assert.equal(board.adjacentStage('brainstorming', -1), 'idea');
  assert.equal(board.adjacentStage('implement', +1), 'review');
  assert.equal(board.adjacentStage('review', +1), 'done');
});
test('adjacentStage: boundaries return null', () => {
  assert.equal(board.adjacentStage('idea', -1), null);
  assert.equal(board.adjacentStage('done', +1), null);
});
test('adjacentStage: unknown stage returns null', () => {
  assert.equal(board.adjacentStage('bogus', +1), null);
});
test('adjacentStage: dir normalizes to ±1 (any positive/negative)', () => {
  assert.equal(board.adjacentStage('idea', 5), 'brainstorming');
  assert.equal(board.adjacentStage('brainstorming', -9), 'idea');
});

// ── shouldEndSession (Kill-on-Move-Entscheidung) ─────────────────────
test('shouldEndSession: nur endSession===true + sessionLive===true + Stage-Wechsel', () => {
  // Der einzige true-Fall.
  assert.equal(board.shouldEndSession({ endSession: true, oldStage: 'brainstorming', newStage: 'review', sessionLive: true }), true);
});
test('shouldEndSession: false ohne endSession-Flag', () => {
  assert.equal(board.shouldEndSession({ endSession: false, oldStage: 'brainstorming', newStage: 'review', sessionLive: true }), false);
  assert.equal(board.shouldEndSession({ endSession: undefined, oldStage: 'brainstorming', newStage: 'review', sessionLive: true }), false);
});
test('shouldEndSession: false wenn keine lebende Session', () => {
  assert.equal(board.shouldEndSession({ endSession: true, oldStage: 'brainstorming', newStage: 'review', sessionLive: false }), false);
});
test('shouldEndSession: false wenn Stufe gleich bleibt', () => {
  assert.equal(board.shouldEndSession({ endSession: true, oldStage: 'review', newStage: 'review', sessionLive: true }), false);
});
test('shouldEndSession: nicht-boolesche endSession-Werte sind nicht true', () => {
  assert.equal(board.shouldEndSession({ endSession: 'true', oldStage: 'idea', newStage: 'brainstorming', sessionLive: true }), false);
  assert.equal(board.shouldEndSession({ endSession: 1, oldStage: 'idea', newStage: 'brainstorming', sessionLive: true }), false);
});
