...
import { toggleItem, addItem, deleteItem, finalizeRelease, editItem, moveItem, setSectionVersion } from './roadmap-writer.js';
...
export async function listProjects() {
  ...
  return mapLimit(reg.projects, LIST_CONCURRENCY, async entry => {
    const roadmapPath = join(entry.path, 'ROADMAP.md');
    let roadmap;
    let mtimeMs;
    try {
...
export async function patchProject(id, body) {
  ...
  if (!['toggle', 'delete', 'add', 'edit', 'move', 'set-version'].includes(action)) throw codedError('bad-action');
