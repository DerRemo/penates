// Projekt-Registry für Penates.
//
// Verwaltet ~/.penates/projects.json — eine flache Liste registrierter
...
const REGISTRY_DIR  = process.env.PENATES_REGISTRY_DIR || join(homedir(), '.penates');
...
// ── createProject ─────────────────────────────────────────────────────────────
// Legt ein neues Projekt an: schreibt `CHANGELOG.md` mit Template und registriert
...
//   'path-exists'    — Zielpfad hat bereits eine CHANGELOG.md (oder un-migrierte ROADMAP.md)
...
export async function createProject({ displayName, path: projectPath }) {
  ...
  // Pfad muss existieren und ein Verzeichnis sein — sonst landet CHANGELOG.md
  ...
  const changelogFile = join(projectPath, 'CHANGELOG.md');
  // Existierendes Plan-Dokument (CHANGELOG.md ODER un-migrierte ROADMAP.md) nicht
  // überschreiben — der Boot-Migrator zieht ein ROADMAP.md ohnehin nach CHANGELOG.md.
  for (const name of ['CHANGELOG.md', 'ROADMAP.md']) {
    try {
      await fs.access(join(projectPath, name));
      throw codedError('path-exists', `${name} already exists in this directory`);
    } catch (e) {
      if (e.code !== 'ENOENT') throw e;
      // ENOENT → gut, dieser Kandidat fehlt
    }
  }

  const name = displayName.trim();
  const template = renderChangelogTemplate(name);
  ...
  return mutateRegistry(async (reg) => {
    ...
    // CHANGELOG.md atomar schreiben (tmp + rename), damit halbes File bei
    // Crash zwischen write und Registry-Update nicht entsteht.
    const tmp = changelogFile + '.tmp';
    await fs.writeFile(tmp, template, 'utf8');
    await fs.rename(tmp, changelogFile);
...
// Template-CHANGELOG.md für neu erstellte Projekte. Der Kommentar-Block am
...
// kompatibel — Released / In Development / Changelog sind da, damit das
// Detail-View sofort Add-Buttons in allen Sections rendern kann. Der Backlog
// lebt seit dem Idea-Pipeline-Cutover auf dem globalen Board (lib/board.js),
// NICHT mehr als `## Backlog`-Sektion in diesem Dokument.
function renderChangelogTemplate(displayName) {
  const today = new Date().toISOString().slice(0, 10);
  return `# ${displayName}

Stand: ${today}. Lebendes Planungs-Dokument, gepflegt über Penates.

<!--
  ╔══════════════════════════════════════════════════════════════════╗
  ║  Parser-Contract — gilt für Penates (lib/roadmap.js).     ║
  ...
  Ideen / Backlog leben auf dem Hub-Board, nicht in diesem Dokument.
  ...
  Workflow:
    1. Idee → als Karte aufs Hub-Board (Stufe "Idea").
    2. Plan feature → add as checkbox in "In Development".
    3. Done → check the checkbox (or click in Hub Detail View).
    4. Finalize release → move items from "In Development" to "Released",
