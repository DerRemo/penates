...
import { worktreePathFor, canIsolate, ensureWorktree, removeWorktree, deleteBranch, branchExists } from './worktree.js';
...
function makeRepo() {
  const dir = mkdtempSync(join(tmpdir(), 'pen-wt-'));
  g(dir, 'init', '-b', 'main');
...
// räumt Repo + den zugehörigen .penates-worktrees-Geschwisterbaum
const cleanup = (dir) => {
  rmSync(dir, { recursive: true, force: true });
  rmSync(join(dirname(dir), '.penates-worktrees', basename(dir)), { recursive: true, force: true });
};

test('worktreePathFor: Geschwister-Dir, genested nach Projekt-Ordnername', () => {
  const p = worktreePathFor('/home/u/proj', 'idea-x');
  assert.equal(p, '/home/u/.penates-worktrees/proj/idea-x');
});
...
test('canIsolate: false für Nicht-Repo', () => {
  const d = mkdtempSync(join(tmpdir(), 'pen-norepo-'));
...
test('branchExists: true für existierenden Branch, false für fehlenden', () => {
  const dir = makeRepo();
  try {
    g(dir, 'branch', 'feature/x');
    assert.equal(branchExists(dir, 'feature/x'), true);
    assert.equal(branchExists(dir, 'feature/nope'), false);
  } finally { cleanup(dir); }
});

test('branchExists: false nach branch -d eines gemergten Branches', () => {
  const dir = makeRepo();
  try {
    g(dir, 'branch', 'feature/merged');         // zeigt auf main → gemergt
    assert.equal(branchExists(dir, 'feature/merged'), true);
    g(dir, 'branch', '-d', 'feature/merged');   // -d gelingt (merged)
    assert.equal(branchExists(dir, 'feature/merged'), false);
  } finally { cleanup(dir); }
});

test('branchExists: true — ungemergter Branch überlebt branch -d', () => {
  const dir = makeRepo();
  const wt = worktreePathFor(dir, 'um');
  try {
    ensureWorktree(dir, 'feature/unmerged', 'main', wt);
    writeFileSync(join(wt, 'a.txt'), 'one\nextra\n');
    g(wt, 'add', '-A'); g(wt, 'commit', '-m', 'unmerged work');
    removeWorktree(dir, wt);                     // Branch aus dem Worktree freigeben
    deleteBranch(dir, 'feature/unmerged');       // git branch -d → verweigert (ungemergt), geschluckt
    assert.equal(branchExists(dir, 'feature/unmerged'), true);  // überlebt
  } finally { cleanup(dir); }
});
