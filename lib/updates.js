...
export const CLI_TARGETS = [
  ...
  { id: 'cli:opencode', name: 'opencode', bin: 'opencode', npm: 'opencode-ai' },
];
...
export function collectHub(hubState) {
  const s = hubState || {};
  return {
    id: 'hub', category: 'hub', name: 'Penates',
...
export function createUpdates({
  ...
  // TTL-Cache mit In-Flight-Dedup: ein Doppelklick auf „Refresh" (refresh:true
  // umgeht das TTL) startete sonst zwei parallele npm+brew+fetch-Builds.
  ...
  return {
    ...
    async getAll({ refresh = false } = {}) {
      return cache.get(build, { force: refresh });
    },
  };
}
