...
import { toggleItem, deleteItem, addItem, addDoneItem, sectionExists, finalizeRelease, editItem, moveItem, setSectionVersion, reorderItem } from './roadmap-writer.js';
...
test('sectionExists matches a versioned In Development header', () => {
  assert.equal(sectionExists('## In Development: v0.7.2\n\n- [ ] x\n', 'dev'), true);
});
test('sectionExists rejects a versionless In Development header (matches addDoneItem)', () => {
  assert.equal(sectionExists('## In Development:\n\n- [ ] x\n', 'dev'), false);
  // and addDoneItem indeed throws on that same content:
  assert.throws(() => addDoneItem('## In Development:\n', 'dev', 'y'), /section-not-found/);
});
test('sectionExists false when section absent', () => {
  assert.equal(sectionExists('## Released: v1\n', 'dev'), false);
  assert.equal(sectionExists('', 'dev'), false);
});
