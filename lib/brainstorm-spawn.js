...
// anweisen. $PENATES_URL/$PENATES_TOKEN bleiben Shell-Literale (Claude löst sie zur
...
export function buildBrainstormPriming(title, cardId) {
  ...
  return `Brainstorme diese Board-Idee und arbeite sie zu einem Spec aus (nutze die brainstorming-Methodik). Idee: "${t}". Wenn der Spec geschrieben + committet ist, verlinke ihn auf der Karte: PATCH $PENATES_URL/api/board/cards/${cardId} mit {"brainstormDoc":"<pfad>"} (Bearer $PENATES_TOKEN).`;
}
...
// (3) nach Bestätigung je Idee eine collab-Karte POSTen. $PENATES_URL/$PENATES_TOKEN
...
// wie $PENATES_URL bleiben dabei literal (Shells re-expandieren das Ergebnis
// einer Expansion nicht) — Claude löst sie später selbst in bash auf.
export function promptedSpawnCommand(cmd) {
  return `${cmd} "$PENATES_PRIME_PROMPT"`;
}
...
// Validiert den abgeleiteten idea/<slug>-Branch als git-Ref. slugifySessionName
// liefert [\w.-] ohne führende/finale Punkte/Striche und ohne Doppel-Striche —
// die einzigen daraus erreichbaren git-check-ref-format-Verstöße sind ein ".."
// (Titel "a..b") und ein ".lock"-Suffix (Titel "x.lock").
export function isValidImplementBranch(branch) {
...
// Muster wie buildBrainstormPriming). $PENATES_URL/$PENATES_TOKEN bleiben Shell-
...
export function buildImplementPriming(card, { isolated = false } = {}) {
  ...
  const patchTail = `Wenn alles fertig + committet ist, advance die Karte als LETZTE Aktion: PATCH $PENATES_URL/api/board/cards/${c.id} mit {"stage":"review","branch":"${branch}","implementSummary":"<2-3 Sätze, was du gebaut hast>"} (Bearer $PENATES_TOKEN). Beende die Session danach NICHT (kein exit) — ich reviewe live.`;
  if (isolated) {
