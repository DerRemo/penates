...
// (Pfad) + branch (idea/<slug>) literal eingebettet (kein Shell-Interp, send-keys -l).
export function buildImplementPriming(card) {
  const c = card || {};
  const clean = (s, n) => String(s || '').replace(/[\u0000-\u001f\u007f]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, n);
  const title = clean(c.title, 200);
  const doc = clean(c.brainstormDoc, 300);
  const branch = implementBranchName(c.title);
  return `Setze diese Board-Idee autonom um. Idee: "${title}". Lies zuerst den Spec unter ${doc}. Lege einen Branch ${branch} vom aktuellen Branch an (existiert er schon, arbeite darauf weiter). Schreibe mit der writing-plans-Methodik einen Plan und setze ihn dann um: TDD, Tests laufen lassen, verifizieren. Committe lokal auf den Branch — KEIN git push. Wenn alles fertig + committet ist, advance die Karte als LETZTE Aktion: PATCH $CC_HUB_URL/api/board/cards/${c.id} mit {"stage":"review","branch":"${branch}","implementSummary":"<2-3 Sätze, was du gebaut hast>"} (Bearer $CC_HUB_TOKEN). Beende die Session danach NICHT (kein exit) — ich reviewe live.`;
}
