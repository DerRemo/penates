...
// Kollisionssicherer Name innerhalb .cch-images/ (Suffix -1, -2, … wie suggestName in files.js).
...
// Stellt sicher dass <cwd>/.gitignore eine eigene Zeile ".cch-images/" hat (idempotent).
...
function ensureGitignore(cwd) {
  try {
    ...
    if (lines.includes('.cch-images/') || lines.includes('.cch-images')) return;
    const prefix = cur && !cur.endsWith('\n') ? '\n' : '';
    appendFileSync(gi, `${prefix}.cch-images/\n`);
  } catch { /* best-effort */ }
}
