// Attention-Engine für Claude Code Hub — Hook-only.
//
...
//
//  - `session-attention` — Notification-wert: feuert nur bei Stop oder
//    Notification, unattached, nicht muted, außerhalb Cool-Down. Triggert
//    Sound/Flash/Unread im Frontend.
//
...
// Per-Session State:
//   { activity, attached, lastHookAt, lastNotifiedAt }
...
export function reportHookEvent(name, event, _payload = {}, now = Date.now()) {
  if (event === 'SessionEnd') { states.delete(name); return; }
