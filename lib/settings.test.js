...
test('get() returns env defaults when no file exists', async () => {
  ...
  assert.deepEqual(settings.get(), { tmuxMouse: 'off', remoteApproval: false, autoRestore: true, autoRestoreContinue: true });
});
...
test('corrupt file is backed up (inside PENATES_HOME) and treated as empty', async () => {
  ...
  await settings.load({ tmuxMouse: 'on', remoteApproval: true });
  assert.deepEqual(settings.get(), { tmuxMouse: 'on', remoteApproval: true, autoRestore: true, autoRestoreContinue: true });
  const files = await fs.readdir(dir);
  assert.ok(files.some(f => f.startsWith('settings.json.corrupt-')), 'corrupt backup landed in temp dir');
});

test('autoRestore + autoRestoreContinue default to true', async () => {
  await settings.load();
  assert.equal(settings.get().autoRestore, true);
  assert.equal(settings.get().autoRestoreContinue, true);
});

test('patch accepts boolean autoRestore/autoRestoreContinue, rejects non-boolean', async () => {
  await settings.load();
  await settings.patch({ autoRestore: false, autoRestoreContinue: false });
  assert.equal(settings.get().autoRestore, false);
  assert.equal(settings.get().autoRestoreContinue, false);
  await settings.patch({ autoRestore: 'yes' }); // invalid → ignored
  assert.equal(settings.get().autoRestore, false);
});
