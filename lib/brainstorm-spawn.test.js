...
import { slugifySessionName, buildBrainstormPriming, resolveBrainstormSpawn, ideaGenSessionName, buildIdeaGenPriming, looksLikeTrustPrompt, implementSessionName, implementBranchName, buildImplementPriming, promptedSpawnCommand } from './brainstorm-spawn.js';
...
test('promptedSpawnCommand appends the env-var reference as a quoted initial prompt', () => {
  assert.equal(promptedSpawnCommand('claude'), 'claude "$CCH_PRIME_PROMPT"');
  assert.equal(
    promptedSpawnCommand('claude --dangerously-skip-permissions'),
    'claude --dangerously-skip-permissions "$CCH_PRIME_PROMPT"');
});
