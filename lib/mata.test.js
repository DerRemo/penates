...
test('parseStatus: real `mata status` output (running)', () => {
  // Verbatim aus echtem `mata status` v1.1.10 auf macOS.
  const raw = [
    'Mata is running.',
    'PID: 48312',
    'Executable: /Applications/Mata.app/Contents/MacOS/MataHostApp',
    'App: /Applications/Mata.app',
    'Version: Mata 1.1.10 (18)',
    'Started: 13. Jun 2026 at 15:04:03',
  ].join('\n');
  const r = mata.parseStatus(raw);
  assert.equal(r.running, true);
  assert.equal(r.pid, 48312);
  assert.equal(r.version, '1.1.10');                 // sauberer Semver, nicht "Mata 1.1.10 (18)"
  assert.equal(r.startedAt, '13. Jun 2026 at 15:04:03');
});

test('parseStatus: real-style not-running phrasing', () => {
  const r = mata.parseStatus('Mata is not running.');
  assert.equal(r.running, false);
  assert.equal(r.pid, undefined);
});
...
// Apple-only feature gating: on Linux (no Mac → no iOS simulator) the Mata
// binary is absent. The feature must gate OFF cleanly — no crash — so the rest
// of the hub runs unaffected. Frontend hides the feature on installed:false.
test('Mata gates cleanly when binary absent (Linux/no-Mac degradation)', async () => {
  freshEnv();
  process.env.MATA_BIN = join(tmpdir(), 'no-mata-on-linux');
  assert.equal(mata.isInstalled(), false);
  assert.equal(await mata.getStatus(), null);
});
