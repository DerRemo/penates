...
export function preflightFinish(repo, branch, base) {
  ...
  try {
    ...
    // merge-tree --write-tree: exit 1 = sauber erkannter Konflikt. Jeder ANDERE
    // Fehler (z.B. git < 2.38 ohne --write-tree, Timeout) heißt: der Preflight
    // konnte NICHT bestätigen, dass der Merge sauber ist. Nicht optimistisch
    // {ok:true} zurückgeben — sonst läuft finishCard in einen Merge, der die
    // Base auf dem direkten Pfad mid-merge in einen Konflikt-State hinterlässt.
    if (typeof e.status === 'number' && e.status === 1) return { ok: false, reason: 'conflict' };
    return { ok: false, reason: 'preflight-error' };
  }
  return { ok: true };
}
...
export function finishCard(repo, branch, base, title, changelogWrite) {
  ...
  // Den Merge IMMER in einem isolierten detached Worktree bauen — nie auf dem
  // Live-Working-Tree/-Index des Repos. Sonst kann ein paralleler externer
  // git-Writer (User im Repo, nicht-isolierter cc-impl-Agent, update.sh) den
  // Merge mid-flight zerschießen (index.lock) ODER fremde gestagte Dateien
  // werden vom `git commit` des Changelogs in denselben Commit gezogen.
  const wt = mkdtempSync(join(tmpdir(), 'pen-finish-wt-'));
  try {
    ...
    // Ergebnis auf `base` anwenden. Ist base ausgecheckt, kann `branch -f` den
    // Ref nicht bewegen → stattdessen ein sauberer Fast-Forward (kann nicht
    // konfliktieren, verweigert auf dirty tree). sha ist per Konstruktion ein
    // Nachfahre von base → der FF ist garantiert. Sonst genügt der Ref-Move.
    if (primary === base) {
      try { git(repo, ['merge', '--ff-only', sha]); } catch (e) { throw stageError('commit', e); }
    } else {
