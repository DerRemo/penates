...
app.patch('/api/board/cards/:id', async (req, res) => {
  ...
  try {
    ...
    // Mein Kill-on-Move (oben) hat sessionRef ggf. schon genullt → finalizeCardToDone
    // killt dann nicht doppelt, räumt aber Worktree + Branch.
    let cleanup;
    if (stage === 'done') cleanup = await finalizeCardToDone(card, auditLog.extractRequestMeta(req));
    const out = enrichCard(card);
    res.json(cleanup ? { ...out, cleanup } : out);
  } catch (e) {
